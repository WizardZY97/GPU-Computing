...
int main(int argc, char **argv)
{
  ...
  args::ValueFlag<int> widthA(parser, "widthA", "Width of matrix A", {"wA"}, defaultSize);
  args::ValueFlag<int> widthB(parser, "widthB", "Width of matrix B", {"wB"}, defaultSize);
  args::ValueFlag<int> heightA(parser, "heightA", "Height of matrix A", {"hA"}, defaultSize);
  args::ValueFlag<int> heightB(parser, "heightB", "Height of matrix B", {"hB"}, defaultSize);
  args::ValueFlag<int> flagAuto(parser, "0/1", "Auto running multiple rounds", {"auto"}, 0);
  args::ValueFlag<int> flagShareMem(parser, "0/1", "Using the shared memory or not", {"shareMem"}, 0);
  ...
  if (args::get(flagAuto) == true)
  {
    ...
    while (i < round)
    {
      ...
      // execute the kernel
      if (args::get(flagShareMem) == true)
        gemm_shared<<<grid, threads>>>(d_C, d_A, d_B, WA, WB);
      else
      ...
      std::cout << WA << "x" << HA << " " << msecTotal << std::endl;
