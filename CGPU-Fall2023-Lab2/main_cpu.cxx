...
int main(int argc, char *argv[])
{
  ...
  args::ValueFlag<int> heightB(parser, "heightB", "Height of matrix B", {"hB"},
                               256);
  args::ValueFlag<int> flagAuto(parser, "heightB", "Height of matrix B", {"auto"},
                                0);
  ...
  int WA, WB, HA, HB, WC, HC, FlagAuto;
  WA = args::get(widthA);
  WB = args::get(widthB);
  HA = args::get(heightA);
  HB = args::get(heightB);
  FlagAuto = args::get(flagAuto);

  if (FlagAuto == true)
  {
    // std::ofstream outputFile("./timing.txt");
    // std::streambuf* originalCoutBuffer = std::cout.rdbuf(outputFile.rdbuf());
    
    int i = 0, round = 7;
    int WA = 16, HA = 16, WB = 16;
    int M = 0, N = 0, K = 0;
    ...
    while (i < round) {
      ...
      std::cout << M << "x" << K << " " << duration.count() << std::endl;
